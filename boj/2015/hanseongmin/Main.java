...
초기에는 구간의 합을 다루는 문제이므로 세그먼트 트리로 생각했지만
세그먼트 트리의 구조 상 답을 구할 수 없는 경우가 존재
따라서 누적합으로 접근. map을 사용해야한다는 아이디어를 떠올리기 어려웠음.
체감 상의 난이도는 G3 정도로 생각
...
현재 원소를 k로 만들 수 있는 값이 누적합 값들의 집합 내에 존재한다면 현재 원소 값을 k로 만들 수 있다.
